...
flags.DEFINE_integer("ls_duration", 20, "Linear start duration [20]")
...
flags.DEFINE_string("task", "1", "Task number for questions to be fetched. See the paper. [1]")

FLAGS = flags.FLAGS


def main(_):
    prefix = "qa%s_" % FLAGS.task
    train_ds, test_ds = data.read_babi(FLAGS.data_dir, prefix=prefix)
    ...
    with tf.Session(graph=graph) as sess:
        ...
        print "Task %s test error: %.2f%%" % (FLAGS.task, (1-acc) * 100)
